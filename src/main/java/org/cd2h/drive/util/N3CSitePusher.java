...
public class N3CSitePusher extends Sheet {
	...
	PreparedStatement stmt = conn.prepareStatement("select * from n3c_maps.sites");
	...
	logger.info("skip count: " + prop_file.getProperty("sites.skipcount"));
	StringBuffer buffer = new StringBuffer(prop_file.getProperty("sites.sheets")
	...
	return Integer.parseInt(prop_file.getProperty("sites.skipcount")) + 1;
    }

    static int getEndRow(List<List<Object>> values) {
	return Integer.parseInt(prop_file.getProperty("sites.skipcount")) + values.size();
    }
