...
public class Sheet extends GoogleAPI {
	...
	public static void main(String... args)
			throws IOException, GeneralSecurityException, SQLException, ClassNotFoundException {
	...
	static public UpdateValuesResponse updateValues(String spreadsheetId, String range, String valueInputOption,
		...
		Sheets service = new Sheets.Builder(HTTP_TRANSPORT, JSON_FACTORY, getCredentials(HTTP_TRANSPORT, SCOPES,
				prop_file.getProperty("sheets.credentials"), prop_file.getProperty("sheets.tokens")))
	...
	static void rebuildDriveSheetAsTable(String sheet, String table)
		...
		if (doTruncate != null && doTruncate.equals("yes")) {
			...
			PreparedStatement dropStmt = conn
					.prepareStatement("drop table if exists " + schema + "." + table + " cascade");
			dropStmt.execute();
			dropStmt.close();
		}
...
//		logger.debug("\tinstitution: " + row.get(4));
				...
				for (int i = row.size() + 1; i <= rowCount; i++) { // the API shorts us sometimes when rightmost cells
																	// are empty
